...
public class ClassSubjectAdapter extends RecyclerView.Adapter<ClassSubjectAdapter.MyViewHolder>{
    ...
    private  List<Pair<Long, String>> strings;
    ...
    public ClassSubjectAdapter(Context context, List<ClassSubjectClass> offerList) {
        this.OfferList = offerList;
        this.context = context;
        strings = new ArrayList<>();


        for(ClassSubjectClass classSubject : offerList){
            for (SubjectClass sub : classSubject.subjects){
                long subjectId = sub.id;
                String title = classSubject.name + "-" + sub.name;
                strings.add(new Pair<>(subjectId, title));
            }
        }
    ...
    @Override
    public ClassSubjectAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_subject, parent, false);


        return new ClassSubjectAdapter.MyViewHolder(itemView);


    }

    @Override
    public void onBindViewHolder(@NonNull final ClassSubjectAdapter.MyViewHolder holder, int position) {
        final Pair<Long, String> item = strings.get(position);
        String title =  item.second;
